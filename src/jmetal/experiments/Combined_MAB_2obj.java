...
import hyperheuristics.selectors.FRRMAB;
import java.io.File;
...
public class Combined_MAB_2obj {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, JMException, ClassNotFoundException {
        ...
        double alpha, beta;
        //Double[] mutationProb={1.0, 0.5, 0.25, 0.10, 0.05};
        Double[] mutationProb={};
        if (args.length == 10) {
        ...
        for (String filename : softwares) {
            ...
            for(double mutationP : mutationProb){
                builder.addProbability(crossoverProbability, mutationP);
            }
            //escolhe algoritimo
            for (int runs = 0; runs < runsNumber; runs++) {
                ...
                builder.initAlgs();
                HyperHeuristicSelector selector = new FRRMAB(builder.getAlgs(), numObj, maxEvaluations, populationSize, C, D, W);
                AlgorithmHH algorithm;
                for (AlgorithmHH alg : builder.getAlgs()) {
                    System.out.println("Inicializando " + alg.getMethodName());
                    int i = 0;
                    long initialtime = System.currentTimeMillis();
                    while (i < maxEvaluations * initsize) {
                        alg.executeMethod();
